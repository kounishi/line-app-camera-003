# LINE公式アカウント向け ミニアプリ(LIFF)サンプル v0.3 — リッチメニュー6項目版

LINE公式アカウントの **リッチメニュー(6項目)の各ボタンから、ミニアプリ(LIFFアプリ)の該当機能を1タップで直接開く** 構成のサンプルです。
`line-app-camera-002` を元に、リッチメニューの構成(6項目)、ご請求額画面の追加、顧客コード紐付けの方法(券面撮影)を変更した別バージョンです。

ビルド不要の静的Webアプリ(HTML / CSS / JavaScript のみ)で、GitHub Pagesで公開しています。
...
| 項目 | 002 | 003(本リポジトリ) |
|---|---|---|
| リッチメニュー | 7項目 | **6項目(お知らせ / 各種申請 / ご請求額 / 契約状況 / 質問する / 緊急連絡先)**。ミニアプリの4機能とお知らせは「リンク」アクションで1タップ起動、「質問する」だけ「テキスト」アクション |
...
LINE公式アカウント トーク画面
    │
    └─ リッチメニュー(6ボタン)
          ├─ お知らせ    [リンク]   → JAHICトピックス …………………… https://jahic-etc.com/info/
          ├─ 各種申請    [リンク]   → ミニアプリ ?screen=apply     (要 顧客コード紐付け)
          ├─ ご請求額    [リンク]   → ミニアプリ ?screen=billing   (要 顧客コード紐付け)
          ├─ 契約状況    [リンク]   → ミニアプリ ?screen=contract  (要 顧客コード紐付け)
          ├─ 質問する    [テキスト] → 「質問があります」をトークに送信 → AIチャット(Webhook)や応答メッセージが返答
          └─ 緊急連絡先  [リンク]   → ミニアプリ ?screen=contacts
...
「リンク」アクションはタップした瞬間にミニアプリ(またはWebページ)が開き、トーク画面にはメッセージが残りません。
トーク上にメニュー名を残したい場合は、ボタンを「テキスト」アクションにして応答メッセージでリンクを返す方式(2タップになる)か、リンクで開いたミニアプリ側から `liff.sendMessages()` でメニュー名を投稿する方式に変更できます(下記「別の起動方式」)。

## リッチメニューの設定値

LINE Official Account Manager(https://manager.line.biz/) → ホーム → トークルーム管理 → リッチメニュー で作成します。
テンプレートは「大 / 6分割(2行×3列)」を選び、`assets/richmenu.png` (元ファイル: `リッチメニュー01.png`)を背景画像にします。

| 位置 | ボタン | アクション種別 | 設定値 |
|---|---|---|---|
| 上段 左 | お知らせ | リンク | `https://jahic-etc.com/info/` |
| 上段 中 | 各種申請 | リンク | `https://miniapp.line.me/2011584909-h9hK7SbT?screen=apply` |
| 上段 右 | ご請求額 | リンク | `https://miniapp.line.me/2011584909-h9hK7SbT?screen=billing` |
| 下段 左 | 契約状況 | リンク | `https://miniapp.line.me/2011584909-h9hK7SbT?screen=contract` |
| 下段 中 | 質問する | テキスト | `質問があります` |
| 下段 右 | 緊急連絡先 | リンク | `https://miniapp.line.me/2011584909-h9hK7SbT?screen=contacts` |

- URLには `src/config.js` の `LIFF_ID`(`2011584909-h9hK7SbT`)を埋め込んでいます。LIFF ID を変えた場合は合わせて修正してください。
- ミニアプリのマイページ「リッチメニュー設定値(開発用)」を開くと、設定中の LIFF ID を埋め込んだ上記の値がそのまま表示されるので、そこからコピーできます。
- この定義はミニアプリ側の `src/data/richMenu.js` にも同じ内容で持っており、ホーム画面のタイルにも使っています。ボタン構成を変える場合は両方を揃えてください。
- 「リンク」アクションの「アクションラベル」(音声読み上げ用)にはボタン名を入れてください。

### 「質問する」(AIチャット)と応答メッセージについて

「質問する」だけはテキストアクションで、`質問があります` がトークに送信されます。これに返答するのはトーク画面のAIチャット(Messaging API の Webhook)で、AIで回答を生成して返信するサーバー側の仕組みです。静的ホスティングのミニアプリでは実装できないため、本リポジトリには含めていません。別途Webhookサーバー(Node.js / Python / Azure Functions など)を用意してください。

Webhookを用意するまでの間は、LINE Official Account Manager → ホーム → トークルーム管理 → **応答メッセージ** で、キーワード `質問があります`(完全一致)に対して次のような文面を登録しておくと、既定の「このアカウントでは個別のお問い合わせを受け付けておりません」が返るのを防げます。

> ご質問をこのトークにそのまま入力してください。担当者(AIチャット)がお答えします。

既定の応答メッセージ(キーワードなしの「Default」)も、「メニューからお選びください。ご質問は「質問する」からどうぞ。」のような案内文に変えておくことをおすすめします。
Webhook を有効にしたら、設定 → 応答設定 で Webhook をオンにし、応答メッセージ側の `質問があります` は削除するか、Webhook 側で同じ返答をするようにします。
...
| 方式 | 動き | 長所 / 短所 |
|---|---|---|
| **リンク(本リポジトリの採用方式)** | タップ → ミニアプリが直接開く | 1タップ。トークにメッセージは残らない |
| テキスト + 応答メッセージ | タップ → メニュー名がトークに送信 → 応答メッセージのリンクをタップ → ミニアプリが開く | トークに操作履歴が残る。2タップになる。サーバー不要 |
| リンク + `liff.sendMessages()` | タップ → ミニアプリが開き、起動時にミニアプリがメニュー名をトークへ投稿 | 1タップで履歴も残る。Scope `chat_message.write` と、初回のメッセージ送信許可が必要。1対1トークから起動したときのみ |

LINE の仕組み上、公式アカウントからの返信メッセージでミニアプリを自動的に開くことはできません(起動には必ずユーザーのタップが必要です)。

## LINE IDと顧客コードの紐付け

各種申請・ご請求額・契約状況は、LINEアカウント(userId)と当社(JAHIC)の顧客データにある顧客コードの紐付けが済んでいないと利用できません。
未紐付けのユーザーがこれらを開くと(リッチメニューからの `?screen=apply` などの直接起動も含む)、**「顧客コードの紐付けが必要です」という案内画面** を表示し、「紐付けを行う」から紐付け画面へ誘導します。紐付けが完了すると、元の画面(各種申請など)へ自動的に進みます。紐付け済みなら該当画面がそのまま開きます。
...
`http://localhost:3000/?mock=1#billing` のようにハッシュで画面を直接開けます(リッチメニューからの `?screen=billing` と同じ動き)。
...
`main` ブランチのルートをそのまま配信しています。更新はコミットしてプッシュするだけで約1分後に反映されます。
...
反映が遅い場合はGitHub Pagesのキャッシュが最大10分残るため、少し待つかスーパーリロードしてください。
...
スマートフォンのLINEで `https://miniapp.line.me/2011584909-h9hK7SbT` を開く(自分にトークで送ってタップ)か、公式アカウントのリッチメニューから起動します。
...
| 項目 | 設定値 |
|---|---|
| チャネル種別 | LINEミニアプリ(公式アカウントの Messaging API チャネルと同じプロバイダーに作成する) |
| サイズ | Full |
| エンドポイントURL | `https://kounishi.github.io/line-app-camera-003/` |
| Scope | `profile`、`openid` |
| ボットリンク機能 | On (Normal) または On (Aggressive) |

発行された本番用の LIFF ID を `src/config.js` の `LIFF_ID` に設定します。
...
src/
  ...
  data/
    richMenu.js               リッチメニュー6項目の定義(アクション種別・開く画面・URL)
